    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if exito:
            ...
            await interaction.followup.send(
                "¿Tenés el link de tu ficha en Nivel20? Hacé clic para agregarlo (opcional):",
...
class Personajes(commands.Cog):
    ...
    async def crear_personaje(self, ctx):
        ...
        if await obtener_personaje(user_id):
            await ctx.send("❌ Ya tenés un personaje creado. Usá `!actualizar_personaje` para editarlo.")
            return
        view = _BotonModal(CrearPersonajeModal(), label="📝 Abrir Formulario", style=discord.ButtonStyle.primary)
        await ctx.send("Hacé clic para crear tu personaje:", view=view)
    ...
    async def actualizar_personaje_cmd(self, ctx):
        ...
        if not personaje:
            await ctx.send("❌ No tenés un personaje creado. Usá `!crear_personaje`.")
            return
    ...
    async def ficha(self, ctx, link: str):
        ...
        if not await obtener_personaje(user_id):
            await ctx.send("❌ Primero creá tu personaje con `!crear_personaje`.")
            return
    ...
    async def ver_personaje(self, ctx, miembro: discord.Member = None):
        ...
        if not personaje:
            nombre = "vos" if objetivo == ctx.author else objetivo.display_name
